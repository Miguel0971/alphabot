...
exports.run = async (bot, message, args, client) => {
   ...
   let fam = await db.get(`fam_${message.author.id}`);
    ...
    let author = await db.fetch(`coletado2_${message.author.id}`);
  
    let quant = await db.get(`spin2_${message.author.id}`)
    if(!quant) quant = 0;
...
🥛﹒⺡﹒◟✦﹕Caso queira saber mais sobre sua família, consulte o chat <#969353834541482004> `)
    ...
    .setImage(message.author.displayAvatarURL())
      ...
      db.subtract(`spin2_${message.author.id}`, 1)
      }
  ...
  if(argumento === 'daily'){
        if(author !== null && timeout - (Date.now() - author) > 0){
          ...
          let amount = 1;
          db.add(`spin2_${message.author.id}`, amount)
            db.set(`coletado2_${message.author.id}`, Date.now())
